...
class GalleryViewScreen extends StatefulWidget {
  const GalleryViewScreen({required this.orgDetails, required this.initialIndex, super.key});
  ...
  Widget build(BuildContext context) {
    return BlocListener<OrganizationListingAndDetailsCubit, OrganizationListingAndDetailsState>(
      ...
      listener: (context, state) {
        state.details.fold(() {}, (t) => t.fold((l) {}, (r) => setState(() => photos = r.photos ?? [])));
      },
      child: Scaffold(
        backgroundColor: Colors.white,
        appBar: AppBar(
          ...
          actions: photos.isNotEmpty ? [IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red), tooltip: 'Delete', onPressed: _onDeleteImage)] : null,
        ),
        body: SafeArea(
          child: Column(
            children: [
                      ...
                      photos.isEmpty
                          ? Center(
                            child: Column(
                              ...
                              children: [
                                Text('No Photos Available!', style: AppStyles.text16Px.poppins.w500.textGrey),
                                const SizedBox(height: 16),
                                FilledButton.icon(
                                  onPressed: () {
                                    AddGymGalleryPhotosSheet(orgDetails: widget.orgDetails).show(context, _cubit);
                                  },
                          ...
                          : PageView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return ClipRRect(borderRadius: BorderRadius.circular(24), child: ImageNetwork(photos[index].image ?? '', width: double.infinity, height: double.infinity));
                            },
                          ),
                ),
              ),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: ClipRRect(borderRadius: BorderRadius.circular(12), child: ImageNetwork(photos[index].image ?? '', width: 70, height: 70)),
                      ),
                    );
                  },
                ),
