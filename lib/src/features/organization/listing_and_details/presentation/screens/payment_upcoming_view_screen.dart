  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >=
        ...
        _cubit.fetchExpiringMemberShip(isPagination: true);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<MembersAndLeadsCubit, MembersAndLeadsState>(
        buildWhen: (p, c) => p.upComingPayments != c.upComingPayments,
        builder: (context, state) {
          return state.upComingPayments.data.fold(
            () => const Center(child: CircularProgressIndicator()),
            (either) => either.fold(
              (error) => error.maybeWhen(
                network: (e) => ErrorUi.network(onTap: _fetch),
                    ...
                    children: [
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemCount:
                              data.results.length +
                              (state.upComingPayments.isPagination ? 1 : 0),
                          itemBuilder: (context, index) {
                            /// 🔥 Pagination Loader
                            if (index == data.results.length &&
                                state.upComingPayments.isPagination) {
