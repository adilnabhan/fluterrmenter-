...
class SubscriptionPlanChooseCom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(create: (context) => SubscriptionCubit(orgDetails: orgDetails), child: const _SubscriptionPlanChooseCom());
  }
}
...
class _SubscriptionPlanChooseCom extends StatefulWidget {
  ...
  @override
  State<_SubscriptionPlanChooseCom> createState() => __SubscriptionPlanChooseComState();
}

class __SubscriptionPlanChooseComState extends State<_SubscriptionPlanChooseCom> {
                ...
                body: Stack(
                  children: [
                    Positioned(
                      ...
                      right: 32,
                      child: SvgPicture.asset('assets/images/svg/vectors/dumble.svg', fit: BoxFit.scaleDown, height: 40, colorFilter: const ColorFilter.mode(AppColors.dark, BlendMode.srcIn)),
                    ),
                    Positioned(
                      top: context.height * .28,
                      right: context.width * .25,
                      child: SvgPicture.asset('assets/images/svg/vectors/hart.svg', fit: BoxFit.scaleDown, height: 40, colorFilter: const ColorFilter.mode(AppColors.dark, BlendMode.srcIn)),
                    ),
                    Positioned(
                      top: context.height * .38,
                      left: 32,
                      child: SvgPicture.asset('assets/images/svg/vectors/shoe.svg', fit: BoxFit.scaleDown, height: 40, colorFilter: const ColorFilter.mode(AppColors.dark, BlendMode.srcIn)),
                    ),
                    Positioned(
                      top: context.height * .4,
                      right: 16,
                      child: SvgPicture.asset('assets/images/svg/vectors/showing_power.svg', fit: BoxFit.scaleDown, height: 40, colorFilter: const ColorFilter.mode(AppColors.dark, BlendMode.srcIn)),
                    ),
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Spacer(),
                          Text('Choose\nSubscription plan', style: AppStyles.text24Px.poppins.w700.dark),
                              ...
                              return state.plans.fold(() => Column(children: [_buildPlanOptionShimmer(), const SizedBox(height: 16), _buildPlanOptionShimmer()]), (either) {
                                return either.fold(
                                  (error) {
                                    return SizedBox(
                                      width: 100,
                                      child: Button.filled(title: 'Retry', ontap: _fetchPlans, buttonColor: AppColors.primary, style: AppStyles.text14Px.poppins.w600.light, raduis: 10000),
                                    ).center;
                                  },
                                  (plans) {
                          ...
                          const Spacer(),
                          BlocBuilder<SubscriptionCubit, SubscriptionState>(
                            buildWhen: (p, c) => p.plans != c.plans || p.selectedSubscriptionModel != c.selectedSubscriptionModel || p.payment != c.payment,
                            builder: (context, state) {
                              ...
                              return Button.filled(
                                ...
                                disabledButtonColor: AppColors.primary.withAlpha(125),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: isSelected ? const Color(0xffC39191) : Colors.grey.shade300),
        ),
        child: Column(
          children: [
            Row(
              ...
              children: [
                Text(plan?.name ?? '', style: isSelected ? AppStyles.text14Px.poppins.w500.light : AppStyles.text14Px.poppins.w500.dark),
                RichText(
                  text: TextSpan(
                    style: isSelected ? AppStyles.text16Px.poppins.w700.light : AppStyles.text16Px.poppins.w700.dark,
                    children: [
                      if (plan?.discountedPrice?.isNotEmpty ?? false)
                        TextSpan(
                          text: '₹${plan?.perMonthPriceExcludeDiscount.toStringAsFixed(2)}',
                          style: (isSelected ? AppStyles.text14Px.poppins.w300.light : AppStyles.text14Px.poppins.w300.dark).copyWith(decoration: TextDecoration.lineThrough),
                        ),
                      if (plan?.regularPrice?.isNotEmpty ?? false) const WidgetSpan(child: SizedBox(width: 4)),
                      TextSpan(text: '₹${plan?.perMonthPriceIncludeDisount.toStringAsFixed(2)}', style: isSelected ? AppStyles.text16Px.poppins.w700.light : AppStyles.text16Px.poppins.w700.dark),
                    ],
                  ),
                ),
              ],
            ...
            Row(
              children: [
                if ((plan?.discountedPrice?.isNotEmpty ?? false) && (plan?.regularPrice?.isNotEmpty ?? false) && plan?.regularPrice != plan?.discountedPrice) ...[
                  Row(
                    children: [
                      Text('Total ₹${plan?.discountedPrice ?? 00}', style: isSelected ? AppStyles.text12Px.poppins.w400.light : AppStyles.text12Px.poppins.w400.dark),
                      const SizedBox(width: 8),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        decoration: BoxDecoration(color: isSelected ? const Color(0xff713E3E) : AppColors.primary.withOpacity(0.1), borderRadius: const BorderRadius.all(Radius.circular(6))),
                        child: Text(
                          'Save ₹${plan?.totalDiscount ?? 00}',
                          style: isSelected ? AppStyles.text12Px.poppins.w400.light : AppStyles.text12Px.poppins.w400.dark.copyWith(color: AppColors.primary),
                        ),
                      ),
                    ],
                  ),
